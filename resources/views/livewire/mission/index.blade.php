<div class="space-y-6">
    ...
    <div class="bg-neutral-50/50 border border-neutral-200 rounded-md p-4 grid grid-cols-2 md:grid-cols-4 gap-4 divide-y md:divide-y-0 md:divide-x divide-neutral-200">
        <div class="px-2 py-1">
            <span class="text-xs text-neutral-600 font-semibold uppercase tracking-wider block">Total Misi</span>
            <span class="text-2xl font-bold text-neutral-900 mt-1 block font-mono">{{ $totalMissions }}</span>
        </div>
        <div class="px-2 py-1 pt-3 md:pt-1">
            <span class="text-xs text-neutral-600 font-semibold uppercase tracking-wider block">Misi Selesai</span>
    ...
    {{-- Filter & Search Bar --}}
    <div class="bg-neutral-50/50 border border-neutral-200 rounded-md p-3 sm:p-4 space-y-3">
        <div class="flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between">
            ...
            <div class="relative flex-1">
                ...
                <input type="text"
                       ...
                       class="w-full pl-9 pr-3 py-1.5 text-xs font-sans bg-white border border-neutral-200 rounded-badge focus:outline-none focus:border-brand focus:ring-1 focus:ring-brand text-neutral-900 placeholder:text-neutral-500 transition" />
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <select wire:model.live="selectedType"
                        class="text-xs py-1.5 px-2.5 bg-white border border-neutral-200 rounded-badge text-neutral-800 focus:outline-none focus:border-brand focus:ring-1 focus:ring-brand">
                ...
                <div class="inline-flex border border-neutral-200 rounded-badge overflow-hidden" role="group">
                    <button type="button"
                            wire:click="setViewMode('grid')"
                            class="p-1.5 text-xs {{ $viewMode === 'grid' ? 'bg-neutral-200 text-neutral-900 font-semibold' : 'bg-white text-neutral-600 hover:text-neutral-900' }} transition"
                            title="Tampilan Grid">
                    ...
                    <button type="button"
                            ...
                            class="p-1.5 text-xs {{ $viewMode === 'list' ? 'bg-neutral-200 text-neutral-900 font-semibold' : 'bg-white text-neutral-600 hover:text-neutral-900' }} transition border-l border-neutral-200"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach ($missions as $mission)
                ...
                <div wire:key="mission-grid-{{ $mission->id }}"
                     class="relative bg-white border {{ $isPassed ? 'border-brand/35 shadow-2xs' : 'border-neutral-200 hover:border-neutral-300' }} rounded-lg p-5 flex flex-col justify-between hover:shadow-xs transition-all duration-200 group overflow-hidden">
                    ...
                    <div>
                        ...
                        <a href="{{ route('missions.show', $mission->id) }}"
                           class="block text-base font-semibold text-neutral-900 group-hover:text-brand transition-colors duration-150 line-clamp-2">
                            {{ $mission->title }}
                        </a>

                        {{-- Cuplikan Deskripsi Kasus / Soal --}}
                        <p class="text-xs text-neutral-600 mt-2 line-clamp-3 leading-relaxed">
                    ...
                    <div class="pt-3 border-t border-neutral-200 flex items-center justify-between">
                        ...
                        {{-- Tombol Aksi Utama --}}
                        <a href="{{ route('missions.show', $mission->id) }}"
                           class="inline-flex items-center justify-center px-3.5 py-1.5 text-xs font-medium bg-brand text-white rounded-lg hover:bg-brand-dark active:scale-[0.98] transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-brand focus:ring-offset-1">
            ...
            <div class="divide-y divide-neutral-200">
                @foreach ($missions as $mission)
                    ...
                    <div wire:key="mission-list-{{ $mission->id }}"
                         class="p-4 hover:bg-neutral-50/80 transition flex flex-col sm:flex-row sm:items-center justify-between gap-3 group">
                        
                        <div class="flex items-start gap-3 flex-1">
                            <div class="w-8 h-8 rounded-md bg-neutral-50 border border-neutral-200 flex items-center justify-center text-neutral-600 shrink-0 mt-0.5 group-hover:border-brand/40 group-hover:text-brand transition-colors">
                                @if ($isCaseStudy)
                            ...
                            <div class="space-y-1">
                                ...
                                <a href="{{ route('missions.show', $mission->id) }}"
                                   class="text-sm font-semibold text-neutral-900 group-hover:text-brand transition-colors block">
                                    {{ $mission->title }}
                                </a>
                        ...
                        <div class="flex items-center justify-between sm:justify-end gap-3 shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-neutral-100">
                            @if ($isPassed)
                                <span class="text-xs font-medium text-brand-dark bg-brand-tint/60 border border-brand/40 px-2 py-0.5 rounded-[2px]">
                                    Selesai ({{ $attempt->score }})
                                </span>
                            @elseif ($attempt)
                                <span class="text-xs font-medium text-neutral-700 bg-neutral-50 border border-neutral-300 px-2 py-0.5 rounded-[2px]">
                                    Skor {{ $attempt->score }}
                                </span>
                            @else
                                <span class="text-xs text-neutral-600 font-medium bg-neutral-50 border border-neutral-200 px-2 py-0.5 rounded-[2px]">
                                    Belum Mulai
                                </span>
                            @endif

                            <a href="{{ route('missions.show', $mission->id) }}"
                               class="px-3.5 py-1.5 text-xs font-medium bg-brand text-white rounded-lg hover:bg-brand-dark active:scale-[0.98] transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-brand/40 focus:ring-offset-1">
